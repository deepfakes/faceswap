...
class SortYaw(SortPitch):
    """ Sorting mechanism for sorting a face by yaw (left to right). Same logic as sort pitch, but
...
class SortRoll(SortPitch):
    """ Sorting mechanism for sorting a face by roll (rotation). Same logic as sort pitch, but
    with different metric """
    def _get_metric(self, aligned_face: AlignedFace) -> float:
        ...
        for i in tqdm(range(0, img_list_len - 1), desc="Comparing", file=sys.stdout, leave=False):
            ...
            for j in range(i + 1, img_list_len):
                ...
                score = np.sum(np.absolute(
                    (fl2 - fl1).flatten()))  # pyright:ignore[reportAttributeAccessIssue]
                if score < min_score:
                    min_score = score
                    j_min_score = j
    ...
    def _sort_landmarks_dissim(self) -> None:
        ...
        for i in tqdm(range(0, img_list_len - 1), desc="Comparing", file=sys.stdout, leave=False):
            ...
            for j in range(i + 1, img_list_len):
                ...
                fl1 = self._result[i][1]
                fl2 = self._result[j][1]
                score_total += np.sum(np.absolute((fl2 - fl1).flatten()))  # pyright:ignore
            self._result[i][2] = score_total  # pyright:ignore
